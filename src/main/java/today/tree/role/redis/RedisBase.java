...
import today.tree.role.redis.factory.RedisPool;
...
* @Company: ediankai
*/
public class RedisBase implements RedisFactory{

	private static final Logger logger = Logger.getLogger(RedisBase.class);
	
    private boolean conectionBroken = false;
...
//    protected RedisBase (){ }
    ...
    /**
	* redis使用结构调整了下 大家知悉。 使用示例： RedisBase("Data") RedisBase("Market") RedisBase("Cache") " 
	*
	* @param redisType 
	*/
	...
	public RedisBase( ){
    	...
    	String className = RedisPool.class.getPackage().getName() + ".RedisPoolFactory";
    	
		try {
			Class<RedisPool> cls = (Class<RedisPool>)Class.forName( className );
			redisPool = cls.newInstance();
    ...
    public void returnResource(Jedis jedis) {
    	...
    	redisPool.returnResource(jedis, conectionBroken);

    }

	/* (non-Javadoc)
	 * @see com.sdc.utils.redis.RedisFactory#help()
	 */
	@Override
	public void help() {
		...
		System.out.println(
				"提示如下：/r/n" +
		        "基本使用方式: RedisBase(\"Data\") RedisBase(\"Market\") RedisBase(\"Cache\") " +
		        "Data类型：通用数据redis实例  此部分仅限于基础数据类的使用  /r/n" +
			    "Market类型:营销类redis实例 例如：积分、优惠券   /r/n" +
		        "Cache类型：缓存类redis实例  例如：需要缓存数据"
	...
	public List<Object> multiHmget(List<String> keys,String... fields){
	    Jedis jedis = getJedis();
	...
	public List<Object> multiHmget(Map<String,String[]> map){
	    Jedis jedis = getJedis();
	    Pipeline pipeline = jedis.pipelined();
